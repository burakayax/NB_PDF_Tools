    ...
    <header className="fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-between border-b border-white/[0.1] bg-gradient-to-r from-nb-bg/90 via-nb-bg-elevated/92 to-nb-bg/90 px-3 shadow-[0_4px_28px_-6px_rgba(0,0,0,0.5)] backdrop-blur-xl backdrop-saturate-150 md:px-6">
      ...
      {subscriptionStatus ? (
        <div className="flex min-w-0 flex-1 items-center justify-center gap-1.5 px-0.5 sm:gap-2 sm:px-2">
          ...
          >
            <span className="max-w-[min(100%,min(260px,100vw-10rem))] truncate rounded-full border border-white/[0.08] bg-nb-panel/60 px-2 py-1 text-center text-[10px] font-semibold leading-snug tracking-wide text-sky-200/95 sm:max-w-[min(100%,min(320px,100vw-12rem))] sm:px-3 sm:text-[11px]">
          ...
          {upgradeVisible ? (
            <button
              ...
              className="nb-transition shrink-0 rounded-full border border-sky-400/40 bg-gradient-to-r from-sky-500/25 to-nb-primary/30 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.06em] text-sky-50 shadow-[0_0_20px_-8px_rgba(56,189,248,0.55)] hover:border-sky-300/55 hover:from-sky-500/35 hover:to-nb-primary/40 sm:px-3 sm:text-[11px]"
            >
              {W.navbarUpgrade}
            </button>
          ) : null}
