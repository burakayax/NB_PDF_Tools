...
3. **Derlemeyi çalıştırın** (yine proje kökünde):
   ...
   ```bash
   pyinstaller --noconfirm --onefile --windowed --name "NB_PDF_Tools" build_entry.py
   ```
...
| Çıktı | Konum |
|--------|--------|
| Yürütülebilir dosya | `dist\NB_PDF_Tools.exe` (veya `--name` ile verdiğiniz isim + `.exe`) |
| Ara dosyalar | `build\` klasörü (gerekirse silebilirsiniz) |

Tam yol örneği: `NB_PDF_Tools\dist\NB_PDF_Tools.exe`
