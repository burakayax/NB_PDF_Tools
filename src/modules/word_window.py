...
class WordWindow(ctk.CTkToplevel):
    def __init__(self, master, ortalama_func, engine, success_dialog_class, access_controller=None):
        ...
        self.title(t("word.window_title"))
    ...
    def run_conversion(self):
        save_path = filedialog.asksaveasfilename(parent=self, title=t("word.save_title"),
                                                 defaultextension=".docx",
                                                 filetypes=[(t("word.save_filetype"), "*.docx")])
        if save_path:
            self.btn_convert.configure(state="disabled", fg_color=self.ui["panel_alt"])
            q = Queue()
            finished = {"value": False}

            progress_dialog = ProgressDialog(self, self.ortalama_func, total_count=3, title=t("word.progress_title"))
            ...
            def worker():
                try:
                    if self.access_controller:
                        self.access_controller.authorize_operation("pdf-to-word", [self.selected_file])
                ...
                try:
                    while True:
                        ...
                        if kind == "progress":
                            ...
                            finished["value"] = True
                            progress_dialog.destroy()
                            messagebox.showerror(t("app.error"), t("word.conversion_failed", error=msg[1]))
